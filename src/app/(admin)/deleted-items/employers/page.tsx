...
export default function DeletedEmployersPage() {
  const [items, setItems] = useState<DeletedItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");

  useEffect(() => {
    fetchDeletedEmployers();
  }, []);

  const fetchDeletedEmployers = async () => {
    try {
      setLoading(true);
      const response = await getDeletedItems("employers");
      const data = response && typeof response === "object" && "data" in response ? (response as any).data : response;
      setItems(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error("Failed to fetch deleted employers:", error);
      toast.error("Failed to load deleted employers");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRestore = async (id: number) => {
    try {
      await restoreItem("employers", id);
  ...
  const handlePermanentDelete = async (id: number) => {
    if (!confirm("Are you sure you want to permanently delete this employer?")) return;
    try {
      await permanentDelete("employers", id);
  ...
  const columns = [
    ...
    { 
      key: "deleted_by", 
      title: "Deleted By", 
      render: (v: unknown) => <Badge variant="danger" className="text-[10px] uppercase font-semibold">{typeof v === "string" && v ? v : "Admin"}</Badge> 
    },
    { 
      key: "actions", 
      title: "Actions", 
  ...
  return (
    <div className="space-y-6 pb-12 antialiased">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-surface-900 tracking-tight">Deleted Employers</h1>
          <p className="text-[13px] text-surface-400 font-medium italic">Archived institute profiles</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-xs overflow-hidden">
        ...
        <DataTable 
          ...
          compact={true} 
          loading={loading}
          emptyMessage="No deleted employers found"
        />
      </div>
    </div>
  );
