...
export default function DeletedJobSeekersPage() {
  ...
  useEffect(() => {
    fetchDeletedJobSeekers();
  }, []);

  const fetchDeletedJobSeekers = async () => {
    try {
      setLoading(true);
      ...
      console.error("Failed to fetch deleted job seekers:", error);
      toast.error("Failed to load deleted job seekers");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRestore = async (id: number) => {
    try {
      await restoreItem("job-seekers", id);
      toast.success("Job seeker restored successfully");
      setItems(prev => prev.filter(item => item.id !== id));
    } catch (error) {
      toast.error("Failed to restore job seeker");
    }
  };

  const handlePermanentDelete = async (id: number) => {
    if (!confirm("Are you sure you want to permanently delete this job seeker?")) return;
    try {
      await permanentDelete("job-seekers", id);
      toast.success("Job seeker permanently deleted");
      setItems(prev => prev.filter(item => item.id !== id));
    } catch (error) {
  ...
  const columns = [
    ...
    { 
      key: "deleted_by", 
      title: "Deleted By", 
      render: (v: string) => <Badge variant="default" className="text-[10px] uppercase font-semibold">{v || "System"}</Badge> 
    },
    { 
      key: "actions", 
      title: "Actions", 
  ...
  return (
    <div className="space-y-6 pb-12 antialiased">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-surface-900 tracking-tight">Deleted Job Seekers</h1>
          <p className="text-[13px] text-surface-400 font-medium italic">Archive of disabled candidate accounts</p>
        </div>
      </div>
